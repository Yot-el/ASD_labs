...
def CreateRuns(A, B, source_file, S):
    '''Объявляем массив, длина S которого равна длине отрезков; На первом шаге читаются S записей и сортируются с помощью внутренней сортировки. Потом поочередно получающиеся массивы записываются то в файл A, то в файл B.'''
    CurrentFile = A
...
def PolyPhaseMerge(A, B, C, D, S):
    ...
    while((os.stat(Input_1)).st_size != 0 and (os.stat(Input_2)).st_size != 0): #Проверка на пустоту файлов с отрезками               
        f_1 = open(Input_1, "r")                                                #Если один из них пустой - в другом находится отсортированный список
        f_2 = open(Input_2, "r")
